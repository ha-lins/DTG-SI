...
flags.DEFINE_string("config_data", "config_data_e2e", "The data config.")
...
def build_model(data_batch, data, step):
    ...
    def get_decoder(cell, y__ref_flag, x_ref_flag, tgt_ref_flag,
        ...
        if attn_flag: # attention
            if FLAGS.attn_x and FLAGS.attn_y_:
                ...
                memory = y_enc_outputs[y__ref_flag]
                memory_sequence_length = y_sequence_length[y__ref_flag]
            elif FLAGS.attn_x:
                memory = x_enc_outputs[x_ref_flag]
                memory_sequence_length = x_sequence_length[x_ref_flag]
            else:
                raise Exception(
    ...
    def beam_searching(cell, y__ref_flag, x_ref_flag, beam_width):
        ...
        decoder, bs_outputs, _, _ = get_decoder_and_outputs(
            ...
            beam_width=None)#config_train.infer_beam_width)
    ...
    step_stage = tf.cast(step, tf.float32) / tf.constant(921.0)
    rec_weight = tf.case([(tf.less_equal(step_stage, tf.constant(1.0)), lambda:tf.constant(1.0)),\
                         (tf.greater(step_stage, tf.constant(1.0)), lambda:FLAGS.rec_w)],\
...
def main():
    ...
    def _eval_epoch(sess, summary_writer, mode):
        ...
        fetches = [
            ...
            # bs_outputs.predicted_ids,  for beam_search
            bs_outputs.sample_id,    # for sample_id
        ...
        while True:
            try:
                ...
                # output_ids = output_ids[:, :, 0] for beam_search
