...
def get_replaced(text00, text01, text02, text10, text11, text12, sent_text):
	"""Combining match and align. All texts must not contain BOS. text 00:x'
	"""
	# print('=============')
	...
	n = len(text00)
	m = len(sent_text)
	# text00 = strip_special_tokens_of_list(text00)
	# print('==============test is:{}, {}, {}, {}, {}, {}, {}'.format(text00, text01, text02, text10, text11, text12, sent_text))
...
def build_model(data_batch, data):
	...
	for ref_flag in [sd_ref_flag, tplt_ref_flag]:
		texts.extend(data_batch['x{}_{}_text'.format(ref_strs[ref_flag], field)][:, 1:-1]
					 for field in x_fields)
	# print('[info]y_strs is:{}'.format(y_strs))
	texts.extend(data_batch['{}_text'.format(field)][:, 1:]
				for field in y_tgt_strs)
...
def main():
	...
	def _eval_epoch(sess, summary_writer, mode):
		...
		while True:
			try:
				target_texts, entry_texts, output_texts = sess.run(fetches, feed_dict)
				target_texts = [
